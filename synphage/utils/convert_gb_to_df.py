...
def genbank_to_dataframe(filename: str):
    ...
    topology, organism, taxonomy = compose(juxt(map(lambda x: mc('get', x, None), annotations_attributes)), at("annotations"))(genome)
    ...
    cds = pl.concat(items=[df_cds, df_cds_pk, df_cds_extract], how="horizontal").rename(mapping={"gene": "cds_gene", "locus_tag": "cds_locus_tag"})
