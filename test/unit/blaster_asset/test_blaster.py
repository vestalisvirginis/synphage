...
def test_file_conversion_positive_no_output():
    ...
    assert [row for row in open('tests/fixtures/synthetic_data/genbank/TT_000001.fna')] == [row for row in open("tests/fixtures/synthetic_data/fasta/TT_000001.fna")]
...
def test_blast_database_positive(tmp_path):
    ...
    assert os.listdir(d) == ['TT_000001.ntf', 'TT_000001.nto', 'TT_000001.not', 'TT_000001.ndb', 'TT_000001.nhr', 'TT_000001.nin', 'TT_000001.nsq']
...
def test_blastn(tmp_path):
    ...
    rs = BLT.get_blastn('tests/fixtures/synthetic_data/fasta/TT_000002.fna', 'tests/temp/test_blast_database_positive0/database/TT_000001', p)
...
def test_parse_blastn(spark, tmp_path):
    ...
    rs = BLT.parse_blastn(spark, 'tests/temp/test_blastn0/blastn_results/TT_000001_vs_TT_000002', p)
    df = spark.read.parquet(p)
    assert len(os.listdir(d)) == 1
    assert set(["query_genome_name", "query_genome_id", "query_gene", "query_locus_tag", "query_start_end", "query_gene_strand", "source_genome_name", "source_genome_id", "source_gene", "source_locus_tag", "source_start_end", "source_gene_strand", 'percentage_of_identity']).issubset(df.columns)
...
def test_extract_locus_tag_gene_positive(spark, tmp_path):
    ...
    assert df.select(F.sum((F.col('gene') == '').cast('integer'))).collect()[0][0] == 0  # No missing gene name
    assert df.select(F.sum((F.col('gene') == '').cast('integer'))).collect()[0][0] == 0  # No missing locus tag
...
def test_extract_locus_tag_gene_multiple_files(spark, tmp_path):
    ...
    df = spark.read.parquet(p)
    assert len(os.listdir(d)) == 1  # 1 file  because data are appended in the same parquet directory
    assert set(['name', 'gene', 'locus_tag']) == set(df.columns)
...
def test_extract_locus_tag_gene_missing_gene_name_value(spark, tmp_path):
    ...
    assert len(os.listdir(d)) == 1
    assert set(['name', 'gene', 'locus_tag']) == set(df.columns)
    assert df.select(F.sum((F.col('gene') == '').cast('integer'))).collect()[0][0] == 1  # One missing gene name
    assert df.select(F.sum((F.col('locus_tag') == '').cast('integer'))).collect()[0][0] == 0  # No missing locus tag
    assert df.count() == 5  # 5 entries
...
def test_extract_locus_tag_gene_missing_locus_tag_value(spark, tmp_path):
    ...
    assert set(['name', 'gene', 'locus_tag']) == set(df.columns)
    assert df.select(F.sum((F.col('gene') == '').cast('integer'))).collect()[0][0] == 0  # No missing gene name
    assert df.select(F.sum((F.col('locus_tag') == '').cast('integer'))).collect()[0][0] == 1  # One missing locus tag
    assert df.count() == 5  # 5 entries
...
def test_gene_presence_table(spark, tmp_path):
    path_locus = 'tests/temp/test_extract_locus_tag_gene_po0/locus_and_gene/locus_and_gene'
...
def test_gene_uniqueness(spark, tmp_path):
    ...
    path_to_dataset = 'tests/temp/test_gene_presence_table0/gene_uniqueness/gene_uniqueness'
    rs = BLT.gene_uniqueness(spark, record_name, path_to_dataset, p)
    df = spark.read.parquet(p)
    assert len(os.listdir(d)) == 1
    assert set(['name', 'gene', 'locus_tag', 'total_seq', 'count', 'perc_presence']) == set(df.columns)