...
def test_database_output_files(mock_env_phagy_dir_blasting):
    context = build_asset_context()
    asset_input = [f"{TEST_DATA_FASTA_FILES}TT_000001.fna"]
    result = create_blast_db(context, asset_input)
    assert isinstance(result, list)
    assert len(os.listdir(TEST_DATA_BLAST_DB)) == 7
    assert os.listdir(TEST_DATA_BLAST_DB) == [
        "TT_000001.ntf",
        "TT_000001.nto",
        "TT_000001.not",
        "TT_000001.ndb",
...
def test_create_blast_db(mock_env_phagy_dir_blasting):
    context = build_asset_context()
    asset_input = [f"{TEST_DATA_FASTA_FILES}TT_00000{i+1}.fna" for i in range(6)]
    result = create_blast_db(context, asset_input)
    assert isinstance(result, list)
    assert len(result) == 6
...
def test_create_blast_db_asset(mock_env_phagy_dir_blasting):
    @asset(name="new_fasta_files")
    def mock_upstream():
        return [f"{TEST_DATA_FASTA_FILES}TT_00000{i+1}.fna" for i in range(6)]

    assets = [create_blast_db, mock_upstream]
