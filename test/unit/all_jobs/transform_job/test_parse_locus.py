...
def test_parse_locus():
    ...
    result = parse_locus(context, PipeConfig(source=SOURCE, target=TARGET, table_dir=TABLES, file="locus_and_gene.parquet"), file='TT_000001.gb')
...
def test_parse_locus_content(spark):
    ...
    assert (
        df.select(F.sum((F.col("locus_tag") == "").cast("integer"))).collect()[0][0] == 0
...
def test_parse_locus_content_missing_gene_name_value(spark):
    context = build_op_context()
    result = parse_locus(context, PipeConfig(source=SOURCE_NEG, target=TARGET, table_dir=TABLES, file="locus_and_gene.parquet"), file='TT_000001_missing_gene_name_value.gb')
...
def test_parse_locus_content_missing_gene_name_key(spark):
    context = build_op_context()
    result = parse_locus(context, PipeConfig(source=SOURCE_NEG, target=TARGET, table_dir=TABLES, file="locus_and_gene.parquet"), file='TT_000001_missing_gene_name_key.gb')
...
def test_parse_locus_content_missing_locus_tag_value(spark):
    context = build_op_context()
    result = parse_locus(context, PipeConfig(source=SOURCE_NEG, target=TARGET, table_dir=TABLES, file="locus_and_gene.parquet"), file='TT_000001_missing_locus_tag_value.gb')
    ...
    assert (
        df.select(F.sum((F.col("locus_tag") == "").cast("integer"))).collect()[0][0] == 1
    )  # No missing locus tag
    assert df.count() == 5  # 5 entries
...
def test_parse_locus_content_missing_locus_tag_key(spark):
    context = build_op_context()
    result = parse_locus(context, PipeConfig(source=SOURCE_NEG, target=TARGET, table_dir=TABLES, file="locus_and_gene.parquet"), file='TT_000001_missing_locus_tag_key.gb')
